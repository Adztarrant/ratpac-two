// Adam T: A encapsulated pmt based off the Toroidal pmt construction model for button (some of the offesets will need adjusted for different pmt this will work for the r7081pe model)
...
EncapsulatedPMTConstruction::EncapsulatedPMTConstruction(DBLinkPtr table, G4LogicalVolume *mother)
  ...
  // Encapsulation 
  fParams.front_encapsulation_material = G4Material::GetMaterial(table->GetS("glass_material"));
  fParams.rear_encapsulation_material = G4Material::GetMaterial(table->GetS("glass_material"));
  fParams.metal_flange_material = G4Material::GetMaterial(table->GetS("glass_material"));
  fParams.acrylic_flange_material = G4Material::GetMaterial(table->GetS("glass_material"));
  fParams.silica_bag_material = G4Material::GetMaterial(table->GetS("glass_material"));
  fParams.cable_material = G4Material::GetMaterial(table->GetS("glass_material"));
  
  fParams.front_encapsulation_surface = Materials::optical_surface[table->GetS("glass_material")];
  fParams.rear_encapsulation_surface = Materials::optical_surface[table->GetS("glass_material")];
  fParams.metal_flange_surface = Materials::optical_surface[table->GetS("glass_material")];
  fParams.acrylic_flange_surface = Materials::optical_surface[table->GetS("glass_material")];
  fParams.silica_bag_surface = Materials::optical_surface[table->GetS("glass_material")];
  fParams.cable_surface = Materials::optical_surface[table->GetS("glass_material")];
...
/*
  ...
  fParams.front_encapsulation_surface = Materials::optical_surface[table->GetS("front_encapsultion_material")];
...
*/
  ...
  try {
    ...
    if (!DB::ParseTableName(waveguide_desc, waveguide_table, waveguide_index)) {
      Log::Die("EncapsulatedPMTConstruction: Waveguide descriptor name is not a valid RATDB table: " + waveguide_desc);
    }
...
G4LogicalVolume *EncapsulatedPMTConstruction::BuildVolume(const std::string &prefix) {
 ...
 // pmtrot->rotateX(angle_x);..
  ...
  //fParams.front_encapsulation_material = G4Material::GetMaterial("nakano_acrylic");
  //fParams.front_encapsulation_surface = Materials::optical_surface[table->GetS("nakano_acrylic")];
  ...
  G4LogicalVolume * front_encapsulation_log=new G4LogicalVolume(
  ...
  G4LogicalVolume * rear_encapsulation_log=new G4LogicalVolume(
						 rear_encapsulation_solid,// G4VSolid
  ...
  G4Tubs* front_metal_flange_solid = new G4Tubs("front_metal_flange_solid",
	  			  ...
	  			  25.3*CLHEP::cm,// rmax
					  0.4*CLHEP::cm,//size z
					  0,CLHEP::twopi);//phi
  G4LogicalVolume * front_metal_flange_encapsulation_log=new G4LogicalVolume(
					 front_metal_flange_solid,// G4VSolid
  ...
  G4Tubs* rear_metal_flange_solid = new G4Tubs("rear_metal_flange_solid",
  				 21.0*CLHEP::cm,// rmin
					 ...
					 0.4*CLHEP::cm,//size z
					 0,CLHEP::twopi);//phi
  G4LogicalVolume * rear_metal_flange_encapsulation_log=new G4LogicalVolume(
					rear_metal_flange_solid,// G4VSolid
					fParams.metal_flange_material, // G4Material
  ...
  G4Tubs* acrylic_flange_solid = new G4Tubs("acrylic_flange_solid",
		      ...
		      0.7*CLHEP::cm,//size z
		      0,CLHEP::twopi);//phi
  G4LogicalVolume * acrylic_flange_encapsulation_log=new G4LogicalVolume(
		     acrylic_flange_solid,// G4VSolid
		     fParams.acrylic_flange_material, // G4Material
		     "acrylic_flange_encapsulation_log");
  G4Box* silica_bag_solid = new G4Box("silica_bag_solid", 
          18*CLHEP::mm, 
          33*CLHEP::mm,
          3*CLHEP::mm);// zhalf                                                                                       
  G4LogicalVolume * silica_bag_encapsulation_log=new G4LogicalVolume(
	       silica_bag_solid,// G4VSolid 
  ...
  front_encapsulation_phys = new G4PVPlacement(0,              // no rotation
                                  G4ThreeVector(0.0, 0.0, -9.8*CLHEP::cm),  // Bounding envelope already constructed to put equator at origin
  ...
  rear_encapsulation_phys = new G4PVPlacement(0,              // no rotation
                                  ...
                                  false,                  // no boolean ops
                                  0);                     // copy number
  front_metal_encapsulaion_flange_phys = new G4PVPlacement(0,              // no rotation
                                  G4ThreeVector(0.0, 0.0, -7.8*CLHEP::cm),  // Bounding envelope already constructed to put equator at origin
                                  front_metal_flange_encapsulation_log,       // the logical volume
                                  prefix + "_encapsulation_phys",  // a name for this physical volume
                                  body_log,           // the mother volume
                                  false,                  // no boolean ops
                                  0);                     // copy number
  rear_metal_encapsulation_flange_phys = new G4PVPlacement(0,              // no rotation
                                  G4ThreeVector(0.0, 0.0, -10.2*CLHEP::cm),  // Bounding envelope already constructed to put equator at origin
                                  rear_metal_flange_encapsulation_log,       // the logical volume
                                  prefix + "_encapsulation_phys",  // a name for this physical volume
                                  body_log,           // the mother volume
                                  false,                  // no boolean ops
                                  0);                     // copy number
  acrylic_flange_encapsulaion_phys = new G4PVPlacement(0,              // no rotation
                                  G4ThreeVector(1.0, 1.0, -9.*CLHEP::cm),  // Bounding envelope already constructed to put equator at origin
                                  acrylic_flange_encapsulation_log,       // the logical volume
                                  prefix + "_encapsulation_phys",  // a name for this physical volume
                                  body_log,           // the mother volume
                                  false,                  // no boolean ops
                                  0);  
  silica_bag_encapsulation_phys = new G4PVPlacement(0,              // no rotation
                                  G4ThreeVector(0.0, 13.5*CLHEP::cm, -17.2*CLHEP::cm),  // Bounding envelope already constructed to put equator at origin
                                  silica_bag_encapsulation_log,       // the logical volume
                                  prefix + "_encapsulation_phys",  // a name for this physical volume
                                  body_log,           // the mother volume
                                  false,                  // no boolean ops
                                  0);  
  cable_encapsulation_phys = new G4PVPlacement(0,              // no rotation
                                  G4ThreeVector(0.0, 0.0, -25*CLHEP::cm),  // Bounding envelope already constructed to put equator at origin
 ...
 // pmtrot->rotateY(angle_y);
 // pmtrot->rotateX(angle_x);
 // G4ThreeVector offsetfrontencapsulation = G4ThreeVector(0.0, 0.0, 0.8*CLHEP::cm);
 // G4ThreeVector offsetfrontencapsulation_rot = pmtrot->inverse()(offsetfrontencapsulation);
  //G4ThreeVector frontencapsulationpos = pmtpos + offsetacrylicflange_rot + offsetfrontencapsulation_rot;
  ...
  if (fParams.simpleVis) {
    ...
    // PMT glass
   // visAtt = new G4VisAttributes(G4Color(0.0, 1.0, 1.0, 0.05));
    visAtt = new G4VisAttributes(G4Color(1.0, 0.0, 0.0, 1.0));
    ...
    central_gap_log->SetVisAttributes(G4VisAttributes::GetInvisible());
    // set encap 
    visAtt = new G4VisAttributes(G4Color(0.2, 0.18, 0.6, 0.5));
    front_encapsulation_log->SetVisAttributes(visAtt);
    visAtt = new G4VisAttributes(G4Color(1., 1., 1., 1.));
    ...
    visAtt = new G4VisAttributes(G4Color(0.0, 1.18, 0.0, 1.0));
...
void EncapsulatedPMTConstruction::CalcInnerParams(GLG4TorusStack *body, std::vector<double> &innerZEdge,
  ...
  ////// The encapsulation size is hard coded in to insure the correct size for BUTTON encapsulation but it could be added here if different pmt sizes were wanted!
  ...
  if (equatorIndex < 0) {
    Log::Die("EncapsulatedPMTConstruction::CalcInnerParams: Pathological PMT shape with no equator edge");
  }
  // sanity check on dynode height
  if (fParams.dynodeTop > innerZEdge[equatorIndex]) {
    Log::Die("EncapsulatedPMTConstruction::CalcInnerParams: Top of PMT dynode cannot be higher than equator.");
  }
}
