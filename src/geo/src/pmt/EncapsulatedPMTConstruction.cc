...
EncapsulatedPMTConstruction::EncapsulatedPMTConstruction(
  ...
  fParams.front_encapsulation_surface =
      Materials::optical_surface[table->GetS("glass_material")];
...
EncapsulatedPMTConstruction::BuildVolume(const std::string &prefix) {
  ...
  if (fParams.useEnvelope) {
    ...
    body_phys = new G4PVPlacement(
        ...
        G4ThreeVector(0.0, 0.0,
                      9.8 * CLHEP::cm), // Bounding envelope already constructed
  ...
  body_region->AddRootLogicalVolume(envelope_log);
  /*GLG4PMTOpticalModel * pmtOpticalModel =*/
  new GLG4PMTOpticalModel(
      prefix + "_optical_model", body_region, envelope_log,
      fParams.photocathode, fParams.efficiencyCorrection, fParams.dynodeTop,
      fParams.dynodeRadius, 0.0, /*prepusling handled after absorption*/
...
EncapsulatedPMTConstruction::NewEnvelopeSolid(const std::string &_name) {
  ...
  G4Sphere *outer_s =
      new G4Sphere(_name + "_main", 0. * CLHEP::mm, 25.4 * CLHEP::cm, 0.0,
                   CLHEP::twopi, 0.0, CLHEP::twopi);
  G4Sphere *inner_s =
      new G4Sphere(_name + "_sub", 0. * CLHEP::mm, 25.3 * CLHEP::cm, 0.0,
                   CLHEP::twopi, 0.0, CLHEP::twopi);
